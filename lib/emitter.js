...
PushtellEventEmitter.prototype.setRandomActiveVariant = function (experimentName, userIdentifier) {
  ...
  var variants = emitter.getSortedVariants(experimentName);
  ...
  var weights = emitter.getSortedVariantWeights(experimentName);
  // Sum the weights
  var weightSum = weights.reduce(function (a, b) {
    return a + b;
  }, 0);
  // A random number between 0 and weightSum
  var weightedIndex = typeof userIdentifier === 'string' ? Math.abs(crc32(userIdentifier) % weightSum) : Math.floor(Math.random() * weightSum);
